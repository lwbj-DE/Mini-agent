...
class ToolRegistry:
    ...
    def set_state(self, state: dict) -> None:
        ...
        Every registered tool's ``set_state`` is called so that tools
        without saved state reset to their default (e.g. todo clears
        its task list for a new session).
        """
        for name, tool in self._tools.items():
            tool.set_state(state.get(name, {}))
